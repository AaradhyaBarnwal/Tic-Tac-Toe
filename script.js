...
boxTag.forEach((box) => {
    box.addEventListener('click', () => {
        console.log(2);
        if (turnO) {
            box.innerHTML = `<h4 class="X">X</h4>`;

            turnO = false;
            chalNa("O");
        } else {
            box.innerHTML = `<h4 class="O">O</h4>`;
    ...
    for (pattern of winPatterns) {
        ...
        if (pos1val !== '' && pos2val !== '' && pos3val !== '') {
            ...
            if (pos1val === pos2val && pos2val === pos3val) {
                console.log(3);
                winnerTag.innerHTML = `<h2>${pos1val} player Wins!!</h2>
                        <h2 class="flower">🌸</h2>
                        <h2 class="congo">Congratulations</h2>
...
function chalNa(players) {
    playerTurnTag.innerHTML = `<h3>Player <span> ${players}</span>'s turn</h3>`;
}
